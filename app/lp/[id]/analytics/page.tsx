...
class ErrorBoundary extends Component<{ children: React.ReactNode }, { hasError: boolean; error: Error | null }> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-[#f4f8fd] p-8">
      ...
      <DashboardLayout pageTitle="LP分析レポート">
        <div className="flex min-h-[50vh] items-center justify-center px-4">
          <div className="text-center bg-white border border-[#e2ebf6] rounded-2xl shadow-sm px-10 py-12">
            <p className="text-lg font-semibold text-[#0b1f3a]">分析データが見つかりませんでした。</p>
        ...
        <div className="px-4 sm:px-6 py-6">
          <div className="mx-auto max-w-6xl space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Link
                href={`/lp/${lpId}/edit`}
                className="inline-flex items-center gap-1.5 rounded-lg border border-[#e2ebf6] bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:border-[#bfe6fb] hover:text-sky-600 transition-colors"
              >
            ...
            <section className="grid grid-cols-2 xl:grid-cols-4 gap-4">
              {[
                ...
                <div key={kpi.label} className="rounded-2xl border border-[#e2ebf6] bg-white shadow-sm p-4 sm:p-5">
                  <p className={`text-2xl font-extrabold tracking-tight tabular-nums ${kpi.accent ? 'text-sky-600' : 'text-[#0b1f3a]'}`}>{kpi.value}</p>
            ...
            {/* Step funnel */}
            <section className="rounded-2xl border border-[#e2ebf6] bg-white shadow-sm p-5 sm:p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-bold text-[#0b1f3a]">ステップファネル分析</h2>
                <span className="text-xs text-slate-500">各ステップの閲覧と離脱状況</span>
              </div>

              {stepFunnel.length === 0 ? (
                <div className="rounded-xl border border-dashed border-[#bfe6fb] bg-[#f8fafc] py-12 text-center text-sm text-slate-500">
                  データがまだ蓄積されていません。
                </div>
              ) : (
                      ...
                      <div key={step.step_id} className="space-y-2">
                        <div className="flex flex-wrap items-center justify-between text-xs text-slate-500 gap-2">
                          <div className="flex items-center gap-2 text-slate-700">
                            <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-[#e9f6fe] text-sky-700 text-[11px] font-bold">
                              {step.step_order + 1}
                            </span>
                        ...
                        <div className="relative h-8 overflow-hidden rounded-xl border border-[#e2ebf6] bg-[#f8fafc]">
                          <div
            ...
            <section className="rounded-2xl border border-[#e2ebf6] bg-white shadow-sm p-5 sm:p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-bold text-[#0b1f3a]">CTAクリック分析</h2>
                    ...
                    return ctaClicks.map((cta, index) => {
                      ...
                      return (
                        <div key={key} className="space-y-2">
                          ...
                          <div className="relative h-6 overflow-hidden rounded-xl border border-[#e2ebf6] bg-[#f8fafc]">
                            <div
            ...
            {/* Public URL */}
            <section className="rounded-2xl border border-[#bfe6fb] bg-[#e9f6fe] p-5 sm:p-6">
              <h3 className="text-sm font-bold text-[#0b1f3a] mb-3">公開URL</h3>
              <div className="flex flex-col sm:flex-row gap-3">
                <input
                  type="text"
                  readOnly
                  value={publicUrl}
                  className="flex-1 rounded-xl border border-[#bfe6fb] bg-white px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-cyan-400/40"
                />
                <div className="flex gap-2">
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 rounded-xl bg-[#0b1f3a] px-4 py-2 text-sm font-bold text-pure-white hover:bg-[#122c4d] transition-colors"
                  >
