...
export default function TokushoPage() {
  ...
  const renderInfoSection = (titleKey: string, items: InfoItem[]) => (
    <section className="rounded-2xl border border-slate-400/45 bg-white/70 p-6 shadow-xl shadow-sky-900/10">
      <h2 className="text-xl font-semibold text-slate-900">{t(titleKey)}</h2>
      <dl className="mt-4 space-y-3 text-sm leading-relaxed">
        {items.map((item, index) => (
          <div key={`${item.label}-${index}`} className="flex flex-col gap-1 rounded-xl border border-slate-400/35 bg-white/60 p-4">
    ...
    return (
      <section className="rounded-2xl border border-slate-400/45 bg-white/70 p-6 shadow-xl shadow-sky-900/10">
        ...
        {paragraphs.map((paragraph, index) => (
          <p key={index} className="mt-4 text-sm leading-relaxed text-gray-800">
            {paragraph}
          </p>
        ))}
      </section>
  ...
  return (
    <DashboardLayout pageTitle={t("pageTitle")} pageSubtitle={t("pageSubtitle")} requireAuth={false}>
      <div className="relative min-h-screen bg-slate-50 text-slate-100">
        ...
        <main className="relative z-10 pt-6 pb-20">
          <div className="container mx-auto max-w-4xl px-4">
            ...
            <div className="space-y-8">
              ...
              <section className="rounded-2xl border border-slate-400/45 bg-white/70 p-6 shadow-xl shadow-sky-900/10">
                <h2 className="text-xl font-semibold text-slate-900">{t("sections.returns.title")}</h2>
              ...
              <section className="rounded-2xl border border-sky-500/50 bg-white/70 p-6 shadow-xl shadow-sky-900/10">
                <h2 className="text-xl font-semibold text-slate-900">{t("sections.updates.title")}</h2>
