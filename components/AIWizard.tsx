          ...
          <div className="grid grid-cols-2 gap-3">
            {options.map((option) => {
              ...
              return (
                ...
                >
                  <span className="flex h-10 w-10 items-center justify-center rounded-md bg-brand-tint text-sky-600">
                    <Icon className="h-5 w-5" aria-hidden="true" />
                  </span>
                  <span className="text-navy-900 text-sm font-semibold leading-tight">{option.label}</span>
                </button>
              );
            })}
          </div>
      ...
      return (
        <div>
          ...
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">商品・サービス名 *</label>
              <input
                value={formData.productName}
                onChange={(e) => setFormData({ ...formData, productName: e.target.value })}
                className="w-full rounded-[10px] border border-line-soft bg-white px-3 py-2 text-sm text-navy-900 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-[3px] focus:ring-sky-500/15"
                placeholder="例：売上を3倍にするLP構築マスター講座"
              />
            </div>

            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">商品説明</label>
              <textarea
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                className="w-full h-20 px-3 py-2 bg-white border border-line-soft rounded-[10px] text-navy-900 text-sm placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-[3px] focus:ring-sky-500/15 resize-none"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-ink-soft mb-1">提供形式</label>
                <input
              ...
              <div>
                <label className="block text-xs font-semibold text-ink-soft mb-1">期間・サポート</label>
                <input
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">得られる変化・約束</label>
              <input
      ...
      return (
        <div>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-ink-soft mb-1">通常価格（円）</label>
                <input
              ...
              <div>
                <label className="block text-xs font-semibold text-ink-soft mb-1">特別価格（円）*</label>
                <input
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">申込締切</label>
              <input
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">CTAボタンのテキスト</label>
              <input
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">限定性・緊急性の訴求</label>
              <input
      ...
      return (
        <div>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-2 flex items-center justify-between">
                <span>申込特典（ボーナス）</span>
                <button
                  type="button"
              ...
              <div className="space-y-2">
                {formData.bonuses.map((bonus, index) => (
                  <div key={index} className="bg-canvas border border-line-soft rounded-[10px] p-3 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      <input
                        value={bonus.title}
                        onChange={(e) => handleUpdateBonus(index, 'title', e.target.value)}
                        className="flex-1 rounded-[8px] border border-line-soft bg-white px-2 py-1 text-xs text-navy-900 placeholder-slate-400 focus:outline-none focus:border-sky-500"
                        placeholder="特典タイトル"
                      />
                      <button
                        type="button"
                    ...
                    <input
                      ...
                      className="w-full rounded-[8px] border border-line-soft bg-white px-2 py-1 text-xs text-navy-900 placeholder-slate-400 focus:outline-none focus:border-sky-500"
                      placeholder="説明"
                    />
                    <input
                      value={bonus.value}
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">返金保証の見出し</label>
              <input
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">保証内容の説明</label>
              <textarea
                value={formData.guaranteeDescription}
                onChange={(e) => setFormData({ ...formData, guaranteeDescription: e.target.value })}
                className="w-full h-16 px-3 py-2 bg-white border border-line-soft rounded-[10px] text-navy-900 text-sm placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-[3px] focus:ring-sky-500/15 resize-none"
    ...
    if (step === 7) {
      return (
        <div>
          ...
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">講師・監修者名</label>
              <input
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">肩書き・実績</label>
              <input
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">プロフィール・経歴</label>
              <textarea
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-[0_40px_100px_-30px_rgba(0,0,0,.6)]">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-extrabold tracking-tight text-navy-900">AIでLPを作る</h2>
            ...
            <button
              onClick={onSkip}
              className="text-slate-500 hover:text-navy-900 transition-colors text-sm font-semibold"
            >
              スキップ →
            </button>
          </div>
            ...
            {[1, 2, 3, 4, 5, 6, 7].map((s) => (
              <div
                ...
                className={`h-1 flex-1 rounded-full transition-all ${
                  s <= step ? 'bg-sky-500' : 'bg-line-soft'
                }`}
              />
            ))}
          </div>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <div className="flex flex-1 gap-3">
            {step > 1 && (
              <button
                ...
                className="px-4 py-2 bg-white border border-line-soft text-navy-900 text-sm font-semibold rounded-[10px] hover:border-tint-border transition-colors"
              >
                ← 戻る
              </button>
            )}
          ...
          <button
            type="button"
            onClick={onSkip}
            className="w-full rounded-[10px] border border-line-soft bg-white px-4 py-2 text-sm font-semibold text-slate-600 transition-colors hover:border-tint-border hover:text-navy-900 sm:w-40"
          >
            後で回答する
          </button>
        </div>
