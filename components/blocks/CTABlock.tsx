...
export default function CTABlock({ content, isEditing, onEdit, productId, fullWidth, onProductClick }: CTABlockProps) {
  ...
  const buttonBaseClass = 'inline-flex items-center justify-center px-6 py-3 sm:px-8 sm:py-4 font-semibold shadow-lg transition-transform hover:scale-[1.03]';
  const buttonClass = fullWidth
    ? `${buttonBaseClass} text-sm sm:text-base md:text-lg rounded`
    : `${buttonBaseClass} text-base sm:text-lg rounded w-full sm:w-auto`;
  const buttonStyle = {
        ...
        {isEditing ? (
          ...
          <>
            ...
            >
              ...
              {content.subtitle && (
                <p
                  className={`text-sm sm:text-base md:text-lg ${fullWidth ? 'mt-1 md:mt-0' : 'mt-1'}`}
