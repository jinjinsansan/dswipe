...
最終更新: 2026-06-12 / Phase1〜19すべて main へマージ済・本番反映済（Phase5以降の詳細はメモリ `redesign-momentum.md` とgitタグ `pre-phaseN-*` 参照）

## 0-i. Phase19（2026-06-12）— UX磨き込み

- **マージ**: main `945deeb`（ブランチ `momentum-phase19-ux2`）。小規模のためタグなし（revert -m 1 で戻せる）。
- **内容**: ①エディタ空状態のオンボーディングカード（ブロックライブラリを開くボタン付き） ②ブロック削除undo（Feedback.tsxのトーストに `actionLabel`/`onAction` 機構を追加、7秒間「元に戻す」） ③モバイルエディタのタブを4等分グリッド化（設定タブの見切れ解消） ④決済完了ページの自動再確認を可視化 ⑤「CTA未設定」→「ボタン未設定」+ツールチップ ⑥サロン詳細の削除パネルを`<dl>`外の独立カードへ ⑦notes一覧ヒーローの言語セレクタ重複削除。
- **残課題（任意）**: admin系alert約30箇所、AIウィザードのsessionStorage脆弱性、売上テーブル横スクロールヒント、「メニュー」アコーディオンの中身予告（i18n要）、巨大ファイル分割（lp edit/PropertyPanel）、notes一覧の旧ロゴサムネ=公式note記事のアイキャッチ差し替え（コンテンツ側・ユーザー作業）。
