...
const noteListMessages: Record<Locale, NoteListMessages> = {
  ja: {
    pageTitle: 'AllNOTES',
    pageSubtitle: '情報発信者が投稿したNOTE記事をここで発見できます',
    filters: {
      all: 'すべて',
      free: '無料記事',
      paid: '有料記事',
  ...
  en: {
    pageTitle: 'All Notes',
    pageSubtitle: 'Discover premium and free NOTE articles from top creators.',
    filters: {
      all: 'All',
      free: 'Free',
      paid: 'Paid',
    ...
    featuredBadge: 'Hot',
    loading: 'Loading...',
    loadError: 'Failed to load NOTE listings.',
    emptyTitle: 'No notes found',
    emptyDescription: 'Try adjusting the filters or using a different keyword. You can also toggle between free and paid notes.',
    emptyBadge: 'New NOTE titles are coming soon',
...
const noteDetailMessages: Record<Locale, NoteDetailMessages> = {
  ja: {
    ...
    fetchError: 'NOTEの取得に失敗しました',
    pointsNotAvailable: 'このNOTEはポイント決済に対応していません',
    yenNotAvailable: 'このNOTEは日本円決済に対応していません',
    ...
    backToNotes: '← AllNOTESへ戻る',
    editNote: 'NOTE編集',
  },
  en: {
    unpublishedLabel: 'Unpublished',
    fetchError: 'Failed to load this note.',
    pointsNotAvailable: 'This note is not available for point purchases.',
    yenNotAvailable: 'This note does not support JPY payments.',
    confirmPointsPurchase: ({ title, pricePoints }) =>
      `Do you want to unlock this note with points?\n\nTitle: ${title}\nPrice: ${pricePoints.toLocaleString()} pts\n\nYour balance will be reduced. Continue?`,
    confirmYenPurchase: ({ title, priceYen }) =>
      `Do you want to purchase this note in JPY?\n\nTitle: ${title}\nPrice: ¥${priceYen.toLocaleString()}\n\nYou will be redirected to the one.lat checkout page. Continue?`,
    pointsPurchaseSuccess: ({ remainingPoints }) =>
    ...
    freeLabel: 'Free',
    authorLabel: 'Author:',
    youAreAuthor: 'This is your note',
    paidContentTitle: 'Premium content ahead',
    paidContentDescription: 'Unlock the remainder of this note to continue reading.',
...
const shareUnlockMessages: Record<Locale, ShareUnlockMessages> = {
  ja: {
    ...
    alreadySharedError: '既にこのNOTEをリツイート済みです。',
  ...
  en: {
    ...
    shareDescription: ({ username, pricePoints }) =>
      `Retweet the official post from @${username ?? 'official'} to read the full note without paying ${pricePoints.toLocaleString()} pts.`,
    ...
    connectHint: 'Link your account to unlock notes by retweeting.',
    errorTitle: 'Error',
    ...
    alreadySharedError: 'You have already retweeted this note.',
...
const purchaseSuccessMessages: Record<Locale, PurchaseSuccessMessages> = {
  ja: {
    heading: '決済が完了しました！',
    description: 'NOTE の購入が正常に完了しました。すぐに記事へ移動して内容をご確認ください。',
    statusLabel: '決済ステータス',
    status: {
      completed: '完了',
      pending: '確認中',
  ...
  en: {
    ...
    description: 'Your purchase was successful. You can now access the full note immediately.',
    ...
    autoRedirect: ({ seconds }) => `You will be redirected to the note in ${seconds} seconds…`,
